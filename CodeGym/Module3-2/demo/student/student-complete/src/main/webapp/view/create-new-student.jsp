...
<div class="container">
    ...
    <form action="/student" method="post">
        <input type="hidden" name="actionStudent" value="createNewStudent" />

        <div class="form-group has-success">
            <label for="id">ID :</label>
            <input type="text" class="form-control" name="id" id="id" value="${student.id}" required/>
            <p>${messageID}</p>
        </div>
                ...
                <c:forEach var="classCodeGym" items="${classList}">
                    <c:choose>
                        <c:when test="${classCodeGym.id.equals(student.idClass)}">
                            <option value="<c:out value='${classCodeGym.id}'/>" selected><c:out value="${classCodeGym.name}"></c:out></option>
                        </c:when>
                        <c:otherwise>
